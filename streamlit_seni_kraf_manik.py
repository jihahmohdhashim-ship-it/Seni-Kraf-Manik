...
# Simpan sebagai `streamlit_seni_kraf_manik.py` dan jalankan dengan:
# streamlit run streamlit_seni_kraf_manik.py
...
with st.sidebar:
    ...
    # Reset data: gunakan expander + checkbox + teks untuk pengesahan (selamat)
    with st.expander("Reset semua data (Hati-hati) 🗑️"):
        st.write("Langkah ini akan memadam semua gambar dan data produk secara kekal.")
        confirm_checkbox = st.checkbox("Saya faham tindakan ini akan memadam semua data.")
        confirm_text = st.text_input("Taip 'DELETE' untuk sahkan:")
        if st.button("Padam Semua Data"):
            if confirm_checkbox and confirm_text == "DELETE":
                # padam gambar
                ...
                st.error("Pengesahan tidak lengkap. Sila centang kotak dan taip 'DELETE' sebelum padam.")
